...
class JarvisUI(QWidget):
    ...
    def __init__(self, tema: str | None = None, voice: VoiceState | None = None):
        ...
        self.timer_repintar.start(16)  # ~60 fps
    ...
    def atualizar_animacao(self):
        try:
            ...
            if not hasattr(self, "_hud_tick"):
                self._hud_tick = 0
                self._hud_neural = 97.4
                self._hud_energy = 3.21
            self._hud_tick = (self._hud_tick + 1) % 25
            if self._hud_tick == 0:
                self._hud_neural = 94.0 + random.random() * 5.0
                self._hud_energy = 3.0 + random.random() * 0.5
    ...
    def paintEvent(self, event):
        ...
        try:
            ...
            # scan lines de fundo (overlay, como no HTML)
            ...
            if iv > 0.04:
                fator = 1.0 if not red else 0.35
                self.desenhar_tentaculos(
                    painter, cx, cy, r_sol, ang_base, t, iv * fator, k
                )

            # núcleo → partículas → arco → títulos
            self.desenhar_nucleo(painter, cx, cy, r_sol, iv, k)
            self.desenhar_particulas(
                painter, cx, cy, r_sol, r_anel2, ang_base, iv, k, red
            )
            self.desenhar_arco(painter, cx, cy, r_anel1, t, k)

            # rotador de varredura (modo scan)
            if self.is_scanning:
                self.desenhar_scan_rotator(painter, cx, cy, r_anel1, t, k)
            ...
            # HUD topo (Sistema Neural / Núcleo / Energia)
            self.desenhar_hud_topo(painter, cx, cy, r_anel3, iv, k, t)

            # cantos decorativos
            self.desenhar_cantos(painter, cx, cy, r_anel3, k)
    ...
    def desenhar_scan_overlay(self, p, W, H, k):
        """Scan lines horizontais de fundo, como no HTML (repeating-linear-gradient)."""
        c = QColor(k.arc)
        c.setAlpha(5)
        p.setPen(QPen(c, 0.6))
    ...
    def desenhar_hud_topo(self, p, cx, cy, r, iv, k, t):
        """HUD superior ancorado ao cx/cy do orb, largura = 2*r_anel3."""
        accent = QColor(k.accent)
        label_c = QColor(accent)
        label_c.setAlpha(140)
        val_c = QColor(accent)
        val_c.setAlpha(220)
        bg_bar = QColor(accent)
        bg_bar.setAlpha(40)

        fnt_lbl = QFont("Courier New", 7, QFont.Weight.Normal)
        fnt_lbl.setLetterSpacing(QFont.SpacingType.AbsoluteSpacing, 2)
        fnt_val = QFont("Courier New", 11, QFont.Weight.Bold)
        fnt_val.setLetterSpacing(QFont.SpacingType.AbsoluteSpacing, 1)

        # âncoras: os 3 blocos se distribuem dentro de cx±r
        left_x = int(cx - r)
        right_x = int(cx + r)
        bar_w = 90
        bar_h = 3
        top = int(cy - r * 1.05) - 38  # logo acima do anel externo

    def desenhar_scan_rotator(self, p, cx, cy, r, t, k):
        """Setor giratório de varredura quando scan mode está ativo."""
        p.save()
        p.translate(cx, cy)
        p.rotate(math.degrees(t * 1.2))
        c = QColor(k.core_mid)
        c.setAlpha(45)
        p.setBrush(QBrush(c))
    ...
    def desenhar_aneis(self, p, cx, cy, r1, r2, r3, t, iv, k):
        # anel externo tracejado (rotativo)
        ...
        # anel interno pontilhado (rotativo inverso)
        ac2 = QColor(k.accent)
        ...
        # 4 pontos orbitais no anel médio
        dot = QColor(k.accent)
    ...
    def desenhar_tentaculos(self, p, cx, cy, r_sol, ang_base, t, iv, k):
        for i in range(12):
            ang = ang_base + math.radians(i * 30)  # 360/12 = 30°
            dist = 180 + math.sin(t * 1.4 + i * 0.9) * 70 + iv * 50 + (i % 3) * 20

            tb = QColor(k.tentacle)
            tb.setAlpha(int(max(0, min(255, 40 + iv * 60 + math.sin(t + i) * 20))))
    ...
    def desenhar_arco(self, p, cx, cy, r, t, k):
        ...
        arc_col.setAlpha(160)
        p.setPen(QPen(arc_col, 2.5, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap))
        p.setBrush(Qt.BrushStyle.NoBrush)
